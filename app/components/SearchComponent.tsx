    ...
    <Dialog>
      <DialogTrigger asChild>
        <div className="rounded-full py-2 px-5 border flex items-center cursor-pointer">
          ...
          <SearchIcon className="text-white bg-primary rounded-full p-1 h-8 w-8" />
        </div>
      </DialogTrigger>
                    ...
                    {getAllCountries().map((item) => {
                      return (
                        <SelectItem key={item.value} value={item.value}>
                          <CountryFlag country={item?.value} /> {item.label} / {item.region}
                        </SelectItem>
                      );
                    })}
                  </SelectGroup>
...
export default SearchModelComponent;
